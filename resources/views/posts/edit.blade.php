<x-app-layout>
    @push('scripts')
    <script src="https://cdn.tiny.cloud/1/dgbmsal1wm6u2ct4605vj5h207kpfgglfz92ma5p6yw12y0i/tinymce/6/tinymce.min.js" referrerpolicy="origin"></script>
    <script>
      tinymce.init({
        selector: '#content',
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('posts.update', $post->id) }}" method="POST">
                                ...
                                @foreach($tags as $tag)
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="tags[]" value="{{ $tag->id }}" 
                                            ...
                                            {{ (collect(old('tags', $post->tags->pluck('id')->toArray()))->contains($tag->id)) ? 'checked' : '' }}>
                        ...
                        <div class="mb-6">
                            ...
                            <x-tinymce name="content" id="content" value="{{ old('content', $post->content) }}" required />
                            @error('content')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
